...
public class AddBookActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        add.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) { addBookToDatabase();}
        });
    }
    ...
    private void addBookToDatabase(){
        ...
        if(bookName.isEmpty() || bookAuthor.isEmpty()) {
            Toast.makeText(this, "А поля кто заполнять будет?", Toast.LENGTH_SHORT).show();
            return;
        }

        long result = db.addBook(bookAuthor, bookName); // TODO: так не должно быть!

        if(result > 0) {
            Toast.makeText(this,"Книгу СЮЮЮДА",Toast.LENGTH_SHORT).show();
